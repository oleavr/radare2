...
R_API bool r_core_bin_set_arch_bits(RCore *r, const char *name, const char *_arch, ut16 bits) {
	...
	if (0 && curfile && curfile->buf && r_buf_size (curfile->buf) > r_io_size (r->io)) {
		// eprintf ("NEW BUFFER %d %d\n", (int)r_buf_size (curfile->buf), (int)r_io_size (r->io));
		RIODesc *d = r_io_open_buffer (r->io, curfile->buf, R_PERM_RWX, 0);
		if (d) {
			r_io_desc_close (desc);
			r_config_set (r->config, "io.va", false);
			// r_core_bin_set_arch_bits(r, NULL, NULL, bits);
